# -*- coding: utf-8 -*-
# run_analysis_all_in_one.py (最終定義版，採用四段式智慧邏輯)
...
# ==============================================================================
#  分析邏輯 (Event Analysis Logic) - 內建全新四段式智慧邏輯
# ==============================================================================
    ...
    for i in range(1, len(all_frames_data)):
        ...
        if state == "SEARCHING_TOSS":
            ...
            if (i - event_candidate['confirm_start_frame']) > frames_to_validate_toss:
                print(f"{log_prefix}  > [第 {i} 幀][重設] 確認期結束，證據不足 ({event_candidate['upward_frames_count']}/{min_upward_confirms})")
            ...
            if event_candidate['upward_frames_count'] >= min_upward_confirms:
                print(f"{log_prefix}  > [第 {i} 幀] 確認拋球成功 (已累積 {event_candidate['upward_frames_count']} 個向上幀) -> 進入 等待頂點與下降 狀態")
                state = "AWAITING_APEX"
            ...
            if (i - event_candidate['toss_start_frame']) > max_frames_to_apex:
                print(f"{log_prefix}  > [第 {i} 幀][重設] 等待球到達頂點超時")
                state = "SEARCHING_TOSS"; continue
            if curr_ball_pos is None:
                event_candidate['lost_frames_count'] += 1
                if event_candidate['lost_frames_count'] > max_lost_frames_apex:
                    print(f"{log_prefix}  > [第 {i} 幀][重設] 等待期間球消失太久 ({event_candidate['lost_frames_count']} 幀)")
                    state = "SEARCHING_TOSS"
                continue
            event_candidate['lost_frames_count'] = 0
            if prev_ball_pos is None: continue
            vy = prev_ball_pos[1] - curr_ball_pos[1]
            if vy < -1:
                print(f"{log_prefix}  > [第 {i} 幀] 偵測到球開始下降 -> 進入 等待擊球 狀態")
                state = "AWAITING_HIT"
                event_candidate['apex_frame'] = i
                event_candidate['lost_frames_count'] = 0
        
        elif state == "AWAITING_HIT":
            if (i - event_candidate['apex_frame']) > max_frames_to_hit:
                print(f"{log_prefix}  > [第 {i} 幀][重設] 等待擊球超時")
                state = "SEARCHING_TOSS"; continue
            if curr_ball_pos is None:
                event_candidate['lost_frames_count'] += 1
                if event_candidate['lost_frames_count'] > 15:
                    print(f"{log_prefix}  > [第 {i} 幀][重設] 等待擊球期間球消失")
                    state = "SEARCHING_TOSS"
                continue
            event_candidate['lost_frames_count'] = 0
            if prev_ball_pos is None: continue
            ...
            if speed > hit_v_thresh:
                ...
                if horizontal_ratio < hit_horizontal_ratio_thresh:
                    print(f"{log_prefix}  > [第 {i} 幀] 偵測到高速位移 (速度: {speed:.1f}, H_ratio: {horizontal_ratio:.1f}) -> 判定為發球！")
                    server_player = None
                    ...
                    print(f"{log_prefix}  > [第 {i} 幀][忽略] 高速移動過於水平 (速度: {speed:.1f}, H_ratio: {horizontal_ratio:.1f})，判定為傳球。")
...
def process_single_video(video_path, base_output_dir, overwrite, frame_args, analysis_args):
    ...
    try:
        ...
        if not overwrite and os.path.exists(final_video_output):
            return {"video": video_base_name, "status": "skipped", "log": "最終輸出檔案已存在。"}
        ...
        analysis_config = {
            "hit_v_thresh": analysis_args['hit_v'], "vertical_ratio_thresh": analysis_args['vertical_ratio'], 
            "toss_vy": analysis_args['toss_vy'], "max_frames_to_apex": analysis_args['max_frames_to_apex'],
            "hit_h_ratio": analysis_args['hit_h_ratio'], "frames_to_validate": analysis_args['frames_to_validate'],
            "min_upward_confirms": analysis_args['min_upward_confirms']
        }
        ...
        return {"video": video_base_name, "status": "success", "log": final_video_output}
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description="[最終定義版] 自動化分析排球影片。")
    ...
    analysis_group.add_argument("--toss_vy", type=float, default=8.0, help="觸發拋球確認的初始最小垂直速度。")
    analysis_group.add_argument("--vertical_ratio", type=float, default=1.5, help="拋球垂直/水平速度的最小比值。")
    analysis_group.add_argument("--hit_v", type=float, default=40.0, help="偵測擊球的最小瞬間速度。")
    analysis_group.add_argument("--hit_h_ratio", type=float, default=2.5, help="擊球時水平速度/垂直速度的最大允許比值。")
    analysis_group.add_argument("--max_frames_to_apex", type=int, default=75, help="等待球從拋起到開始下降的最長幀數。")
    # ✨ --- 新增的關鍵可調參數 --- ✨
    analysis_group.add_argument("--frames_to_validate", type=int, default=8, help="進入確認狀態後，觀察多少幀。")
    analysis_group.add_argument("--min_upward_confirms", type=int, default=3, help="在觀察期內，需要多少幀向上才算成功。")
    ...
    analysis_args = {
        'toss_vy': args.toss_vy, 'vertical_ratio': args.vertical_ratio, 'hit_v': args.hit_v, 
    ...
    # ... (main 函數的其餘部分，包括平行處理和報告生成，都維持不變)
        ...
        with tqdm(total=len(futures), desc="整體進度", unit="video") as pbar:
            for future in as_completed(futures):
                ...
                try: results.append(future.result())
                    ...
                    results.append({"video": os.path.basename(video_path), "status": "failed", "log": f"執行時發生嚴重錯誤: {exc}"})
    print("\n" + "="*80)
